...
function registration(req,res) {
	...
	model.search(req.body.userid,"userid" ,"customers",function (result) {
		if(result.length>0){
			response.userid="the userid is already taken , "
			res.json({ status : 400,response:response})
		}
		else{
			var temp=true;
			if(!validations.validatename(req.body.fname)) {
				response.firstname="the firstname  consists only alphabets , "
				temp=false
			}
			if(!validations.validatename(req.body.lname)) {
				response.lastname="the lastname consists only alphabets ,"
				temp=false
			}
			if(!validations.validateuserid(req.body.userid)) {
				response.userid="the userid  consists only alphanumeric only  ,"
				temp=false;			
			}
			if(!validations.validatepassword(req.body.pwd)) {
				response.password="the password must contain atleast six characters ."
				temp=false				
			}
			if(!validations.matchpassword(req.body.pwd,req.body.cpwd)) {
				response.matchpassword="password and confirm password doent match "
				temp=false				
			}
			if(temp) {
...
function deletecustomer(req,res) {	
	model.search(req.headers.token,"token","customers",function(result){
		if(result.length==0){
			res.json({status:400,response:"not a valid token"})
		}
		else{
			var myquery = { token: req.headers.token};
			model.remove(myquery,"customers");
			var myquery={customerid:result[0]._id};
			model.removeall(myquery,"post")
			res.json({status:200,response:"successfully deleted user "})
		}
	})
}

module.exports = {registration,login,logout,updatecustomerdetails,deletecustomer};